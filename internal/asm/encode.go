...
func sizeToBits(sz Size) uint16 {
	switch sz {
	case SZ_B: return 0x0000
	case SZ_W: return 0x0040
	case SZ_L: return 0x0080
	default: return 0
	}
}
...
func applyField(wordVal uint16, f FieldRef, p *prepared) uint16 {
	...
	case F_BranchLow8:
		if !p.BrUseWord { return wordVal | uint16(uint8(p.BrDisp8)) }
		return wordVal
	default:
		return wordVal
	}
...
func emitTrailer(t TrailerItem, p *prepared) ([]byte, error) {
	switch t {
	case T_SrcEAExt:
		if len(p.SrcEA.Ext) == 0 { return nil, nil }
		out := make([]byte, 0, 2*len(p.SrcEA.Ext))
		for _, w := range p.SrcEA.Ext { out = append(out, byte(w>>8), byte(w)) }
		return out, nil
	case T_DstEAExt:
		if len(p.DstEA.Ext) == 0 { return nil, nil }
		out := make([]byte, 0, 2*len(p.DstEA.Ext))
		for _, w := range p.DstEA.Ext { out = append(out, byte(w>>8), byte(w)) }
		return out, nil
	case T_ImmSized:
		return word(int16(p.Imm)), nil
	case T_BranchWordIfNeeded:
		if p.BrUseWord { return word(p.BrDisp16), nil }
		return nil, nil
	}
	return nil, nil
}
...
func Encode(def *InstrDef, form *FormDef, ins *Instr, sym map[string]uint32) ([]byte, error) {
	...
	if ins.Args.Src.Kind != EAkNone {
		p.SrcEA, err = encodeEA(ins.Args.Src, ins.Size)
		if err != nil { return nil, err }
	}
	if ins.Args.Dst.Kind != EAkNone {
		p.DstEA, err = encodeEA(ins.Args.Dst, ins.Size)
		if err != nil { return nil, err }
	}
	...
	if ins.Args.Target != "" {
		...
		if !ok { return nil, fmt.Errorf("undefined label: %s", ins.Args.Target) }
		p.TargetPC = addr
		// try short (PC+2), else word (PC+4)
		d8 := int32(addr) - int32(p.PC+2)
		if d8 >= -128 && d8 <= 127 && d8 != 0 {
			p.BrUseWord = false; p.BrDisp8 = int8(d8)
		} else {
			d16 := int32(addr) - int32(p.PC+4)
			if d16 < -32768 || d16 > 32767 { return nil, fmt.Errorf("branch displacement out of range") }
			p.BrUseWord = true; p.BrDisp16 = int16(d16)
		}
	}
	...
	for _, step := range form.Steps {
		...
		if haveWord {
			w := step.WordBits
			for _, f := range step.Fields { w = applyField(w, f, &p) }
			out = append(out, byte(w>>8), byte(w))
		}
		for _, tr := range step.Trailer {
			tb, err := emitTrailer(tr, &p); if err != nil { return nil, err }
			out = append(out, tb...)
		}
	}
	return out, nil

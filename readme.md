...
- `FMOVEM` — FPn register-list save/restore, static (`FP0-FP3/FP5`) and dynamic (a `Dn` holding the list at runtime), both directions, plus the `FPCR`/`FPSR`/`FPIAR` control-register list (`--fpu`)
...
- `TBLS`/`TBLSN`/`TBLU`/`TBLUN` (CPU32's table-lookup-and-interpolate instructions — see Known Limitations for a verification caveat unique to these four) (`--cpu cpu32`)
